...
<template>
  <div :class="pageId" v-if="isLogin">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4">
      <router-link v-for="track in recentlyPlayedTracks.slice(0, 8)" :key="track.played_at" :to="`/playlist/${track.track.id}`" class="flex flex-row bg-[#2e2e2e] shadow-sm rounded-lg relative group p-2 items-center hover:bg-[#424242] cursor-pointer">
